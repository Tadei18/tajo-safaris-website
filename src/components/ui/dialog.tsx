...
>(({ className, children, hideCloseButton, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-1/2 top-1/2 z-50 grid w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 bg-surface p-6 shadow-lifted rounded-2xl",
    ...
    >
      ...
      {!hideCloseButton && (
        <DialogPrimitive.Close
          aria-label="Close"
          className="absolute right-4 top-4 rounded-full p-2 text-ink-soft transition hover:bg-sand/60"
        >
          <X className="size-4" />
        </DialogPrimitive.Close>
      )}
